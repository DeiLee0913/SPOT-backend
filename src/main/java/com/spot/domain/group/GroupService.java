...
public class GroupService {
    ...
    public StudyGroup create(Long userId, String rawName) {
        ...
        ensureNotAlreadyEngaged(userId);
    ...
    @Transactional
    public GroupMember join(Long userId, String inviteCode) {
        ...
        ensureNotAlreadyEngaged(userId);
        return memberRepository.save(GroupMember.pending(group.getId(), userId));
    }

    @Transactional(readOnly = true)
    public MyGroup getMyGroup(Long userId) {
        GroupMember membership = currentMembership(userId)
            .orElseThrow(() -> new NotFoundException("NO_GROUP", "소속된 그룹이 없습니다."));
        StudyGroup group = getGroup(membership.getGroupId());
        return new MyGroup(group, membership.getStatus(), group.isCreator(userId));
    }

    @Transactional(readOnly = true)
    public Optional<MyGroup> findMyGroup(Long userId) {
        return currentMembership(userId).map(membership -> {
            StudyGroup group = getGroup(membership.getGroupId());
            return new MyGroup(group, membership.getStatus(), group.isCreator(userId));
        });
    }
    ...
    @Transactional
    public void leave(Long userId, Long successorUserId) {
        GroupMember me = memberRepository.findByUserIdAndStatus(userId, MemberStatus.ACTIVE)
            .orElseThrow(() -> new NotFoundException("NO_GROUP", "소속된 그룹이 없습니다."));
        StudyGroup group = getGroup(me.getGroupId());
    ...
    @Transactional(readOnly = true)
    public List<GroupMember> listPendingRequests(Long creatorUserId) {
        StudyGroup group = creatorGroup(creatorUserId);
    ...
    public GroupMember approve(Long creatorUserId, Long memberId) {
        StudyGroup group = creatorGroup(creatorUserId);
        GroupMember member = getMemberInGroup(memberId, group.getId());
    ...
    private void ensureNotAlreadyEngaged(Long userId) {
        memberRepository.findByUserIdAndStatus(userId, MemberStatus.ACTIVE).ifPresent(m -> {
            throw new ConflictException("ALREADY_IN_GROUP", "이미 그룹에 소속되어 있습니다.");
        });
        memberRepository.findByUserIdAndStatus(userId, MemberStatus.PENDING).ifPresent(m -> {
            throw new ConflictException("ALREADY_REQUESTED", "이미 승인 대기 중입니다.");
        });
    }

    private Optional<GroupMember> currentMembership(Long userId) {
        Optional<GroupMember> active = memberRepository.findByUserIdAndStatus(userId, MemberStatus.ACTIVE);
        if (active.isPresent()) {
            return active;
        }
        return memberRepository.findByUserIdAndStatus(userId, MemberStatus.PENDING);
    }

    private StudyGroup creatorGroup(Long creatorUserId) {
        GroupMember me = memberRepository.findByUserIdAndStatus(creatorUserId, MemberStatus.ACTIVE)
            .orElseThrow(() -> new NotFoundException("NO_GROUP", "소속된 그룹이 없습니다."));
        StudyGroup group = getGroup(me.getGroupId());
